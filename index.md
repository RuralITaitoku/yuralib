- [記録](#記録)
- [ブックマーク](#ブックマーク)
  - [開発](#開発)
  - [仕事](#仕事)
  - [生活](#生活)
- [ツール](#ツール)
  - [yura54](#yura54)
- [その他](#その他)

# 記録
1. 2026年
    1. [予定と実績](./prj/yura54/md/2026_Plan_vs_Actual.md)
    2. [収入と支出](./prj/yura54/md/2026_Cash_Flow.md)

# ブックマーク
## 開発
1. [UTF-8文字コード](https://orange-factory.com/dnf/utf-8.html)
2. Rust
    - [The Rust Programming Language](https://doc.rust-lang.org/stable/book/title-page.html)
## 仕事
1. [Teams](https://teams.microsoft.com/v2/)
## 生活
1. [柳川市の天気](https://tenki.jp/forecast/9/43/8240/40207/10days.html)
2. [夢展望](https://dreamvs.jp/pages/brand_dearmylove_)

# ツール
## yura54
### ブラウザでPVS形式の一覧からひな形の形式でコードを生成するツール。
   1. [PSV⇒コード](./prj/yura54/html/psv_to_code.html)
### ブラウザでPSV形式の一覧をTSV形式に変換してクリップボードにコピーするツール。
   1. [PSV⇒TVS](./prj/yura54/html/psv_to_tsv.html)
### ブラウザでPSV形式の一覧から、数値列の合計を計算してPSV形式で結果の行を出すツール。
   - 結果は、合計行だけ。
   - 結果をそのままクリップボードにコピーして。
   1. [PSV合計](./prj/yura54/html/psv_sum.html)
### ブラウザでアナログ時計を表示するアプリ。
   - 5分おきに読み上げを開始するボタン。
   - ダークモード
   - 時計盤には、時刻の刻印をする。
   1. [アナログ時計0](./prj/yura54/html/analog_clock.html) 
   2. [アナログ時計1](./prj/yura54/html/analog_clock1.html)
   2. [アナログ時計2](./prj/yura54/html/analog_clock2.html)
### ブラウザで開始日付と終了日付を入力して、「- [{{年}}年{{月}}月{{日}}日({{曜日}})]」形式のコードを出力するツール。
   1. [日付生成](./prj/yura54/html/date_generator.html)

# その他

